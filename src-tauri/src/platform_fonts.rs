...
fn font_dir() -> Result<PathBuf, String> {
    ...
    if legacy.is_dir() {
        for source in fs::read_dir(&legacy).into_iter().flatten().filter_map(Result::ok).map(|entry| entry.path()).filter(|path| is_font_file(path)) {
...
fn is_font_file(path: &Path) -> bool {
    path.is_file() && path.extension().and_then(|value| value.to_str()).is_some_and(|value| {
        FONT_EXTENSIONS.iter().any(|extension| value.eq_ignore_ascii_case(extension))
    })
}
...
fn font_family(path: &Path) -> String {
    ...
    let parsed = bytes.as_deref().and_then(|data| {
        ...
        (0..face_count).find_map(|index| {
            ...
            face.names()
                .into_iter()
                .filter(|name| name.name_id == ttf_parser::name_id::TYPOGRAPHIC_FAMILY || name.name_id == ttf_parser::name_id::FAMILY)
...
fn stored_font(path: &Path, imported: bool) -> Result<StoredFont, String> {
    let id = path.file_name().and_then(|value| value.to_str())
...
fn copy_font_into_library(source: &Path) -> Result<StoredFont, String> {
    ...
    let original_name = source.file_name().and_then(|value| value.to_str()).unwrap_or("font.ttf");
    ...
    if destination.exists() {
        ...
        let stem = source.file_stem().and_then(|value| value.to_str()).unwrap_or("font");
...
fn windows_system_fonts() -> Result<Vec<StoredFont>, String> {
    use std::{ffi::OsString, os::windows::ffi::OsStringExt, ptr::{null, null_mut}};
    use windows_sys::Win32::{Foundation::{ERROR_MORE_DATA, ERROR_NO_MORE_ITEMS, ERROR_SUCCESS}, System::Registry::{RegCloseKey, RegEnumValueW, RegOpenKeyExW, HKEY, HKEY_CURRENT_USER, HKEY_LOCAL_MACHINE, KEY_READ, REG_EXPAND_SZ, REG_SZ}};

    let mut fonts = Vec::new();
    let windows_fonts = std::env::var_os("WINDIR").map(PathBuf::from).unwrap_or_else(|| PathBuf::from("C:\\Windows")).join("Fonts");
    let key_path = "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Fonts".encode_utf16().chain(Some(0)).collect::<Vec<_>>();
    for root in [HKEY_LOCAL_MACHINE, HKEY_CURRENT_USER] {
        ...
        for index in 0_u32.. {
            ...
            let mut kind = 0_u32;
            let status = unsafe { RegEnumValueW(key, index, name.as_mut_ptr(), &mut name_len, null(), &mut kind, data.as_mut_ptr() as *mut u8, &mut data_len) };
            if status == ERROR_NO_MORE_ITEMS { break }
            if status == ERROR_MORE_DATA { continue }
            if status != ERROR_SUCCESS || !matches!(kind, REG_SZ | REG_EXPAND_SZ) { continue }
            let family = OsString::from_wide(&name[..name_len as usize]).to_string_lossy().trim().trim_end_matches(" (TrueType)").trim_end_matches(" (OpenType)").to_string();
            data.truncate(data_len as usize / size_of::<u16>());
            let mut file_name = OsString::from_wide(&data).to_string_lossy().trim_end_matches('\0').to_string();
            if kind == REG_EXPAND_SZ {
                if let Some(windows) = std::env::var_os("WINDIR") { file_name = file_name.replace("%WINDIR%", &windows.to_string_lossy()) }
            }
            let candidate = PathBuf::from(&file_name);
            let path = if candidate.is_absolute() { candidate } else { windows_fonts.join(candidate) };
            if family.is_empty() || !is_font_file(&path) { continue }
            fonts.push(StoredFont { id: format!("system:{family}"), family, file_path: path.to_string_lossy().to_string(), imported: false });
        }
...
fn windows_system_fonts() -> Result<Vec<StoredFont>, String> { Ok(Vec::new()) }
...
pub(crate) fn list_fonts() -> Result<FontListing, String> {
    ...
    let mut fonts = fs::read_dir(&directory)
        ...
        .collect::<Vec<_>>();
    fonts.sort_by(|left, right| left.family.cmp(&right.family).then_with(|| left.id.cmp(&right.id)));
    Ok(FontListing { directory_path: directory.to_string_lossy().to_string(), fonts })
}
...
pub(crate) fn import_font() -> Result<Option<StoredFont>, String> {
    let Some(source) = font_dialog().pick_file() else { return Ok(None) };
    copy_font_into_library(&source).map(Some)
}

#[tauri::command]
pub(crate) fn import_system_font(id: String) -> Result<StoredFont, String> {
    let font = windows_system_fonts()?.into_iter().find(|font| font.id == id)
        .ok_or_else(|| "找不到选择的系统字体。".to_string())?;
    copy_font_into_library(Path::new(&font.file_path))
}

fn safe_font_id(id: &str) -> Result<&str, String> {
    if id.is_empty() || id.contains(['/', '\\']) || id.contains("..") || !FONT_EXTENSIONS.iter().any(|extension| id.to_ascii_lowercase().ends_with(&format!(".{extension}"))) {
...
pub(crate) fn delete_font(id: String) -> Result<(), String> {
    ...
    if !path.exists() { return Err("字体文件不存在。".to_string()) }
...
#[cfg(test)]
mod tests {
    use super::fallback_font_family;
    use std::path::Path;

    #[test]
    fn derives_a_readable_family_alias_from_the_file_name() {
        assert_eq!(fallback_font_family(Path::new("C:/Fonts/Moon_Pixel-Bold.ttf")), "Moon Pixel Bold");
    }
}
