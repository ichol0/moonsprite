...
export function AppMenuBar({
  ...
  return <PerformanceProfiler id="AppMenuBar"><header className="topbar">
    ...
    <nav className="menu-strip" aria-label={t('app.menu.mainAria')} onPointerOver={(event) => { const button = (event.target as HTMLElement).closest('button[aria-expanded]'); const item = button?.parentElement; if (item?.parentElement !== event.currentTarget) return; hoverMenuAt(Array.from(event.currentTarget.children).indexOf(item)) }}>
      <div className="menu-item"><button aria-expanded={openMenu === 'file'} onClick={() => toggleMenu('file')}>{t('app.menu.file')}</button>{openMenu === 'file' && <div className="menu-popover"><button onClick={() => { onNew(); closeMenu() }}>{t('app.menu.file.new')} <kbd>{shortcutFor('newDocument')}</kbd></button><button onClick={() => { onOpen(); closeMenu() }}>{t('app.menu.file.open')} <kbd>{shortcutFor('openDocument')}</kbd></button><div className="menu-submenu"><button className="menu-submenu-trigger">{t('app.menu.file.recent')} <ChevronRight size={14} /></button><div className="menu-popover menu-submenu-popover recent-files-submenu">{recentFiles.length === 0 ? <button disabled>{t('app.menu.file.noRecent')}</button> : recentFiles.map((recent) => <button key={recent.filePath} title={recent.filePath} onClick={() => { onOpenRecent(recent.filePath); closeMenu() }}>{recent.fileName}</button>)}</div></div><span className="menu-divider" /><button disabled={!session} onClick={() => { void workspace.saveActive(); closeMenu() }}>{t('app.menu.file.save')} <kbd>{shortcutFor('save')}</kbd></button><button disabled={!session} onClick={() => { onSaveAs(); closeMenu() }}>{t('app.menu.file.saveAs')} <kbd>{shortcutFor('saveAs')}</kbd></button><button disabled={!session} onClick={() => { onExport(); closeMenu() }}>{t('app.menu.file.export')} <kbd>{shortcutFor('exportDocument')}</kbd></button><button disabled={!session?.document.filePath && !session?.document.sourceFilePath} onClick={() => { if (session) onOpenProjectFolder(session.document.id); closeMenu() }}>{t('app.menu.file.openFolder')}{shortcutHint('openProjectFolder')}</button><span className="menu-divider" /><button disabled={!session} onClick={() => { onOpenTimelapse(); closeMenu() }}>{t('app.menu.file.timelapse')}</button><button disabled={!session} onClick={() => { onOpenProjectInfo(); closeMenu() }}>{t('app.menu.file.projectInfo')}</button></div>}</div>
      <div className="menu-item"><button aria-expanded={openMenu === 'edit'} onClick={() => toggleMenu('edit')}>{t('app.menu.edit')}</button>{openMenu === 'edit' && <div className="menu-popover"><button disabled={!session?.history.canUndo} onClick={() => { workspace.undo(); closeMenu() }}>{t('app.menu.edit.undo')} <kbd>{shortcutFor('undo')}</kbd></button><button disabled={!session?.history.canRedo} onClick={() => { workspace.redo(); closeMenu() }}>{t('app.menu.edit.redo')} <kbd>{shortcutFor('redo')}</kbd></button><div className="menu-submenu"><button className="menu-submenu-trigger">{t('app.menu.edit.pasteSpecial')} <ChevronRight size={14} /></button><div className="menu-popover menu-submenu-popover"><button onClick={() => { void workspace.pasteAsNewDocument(); closeMenu() }}>{t('app.menu.edit.pasteAsDocument')}{shortcutHint('pasteAsNewDocument')}</button><button disabled={!session} onClick={() => { void workspace.pasteAsNewLayer(); closeMenu() }}>{t('app.menu.edit.pasteAsLayer')}{shortcutHint('pasteAsNewLayer')}</button></div></div><button disabled={!session || Boolean(session.selectedGroupId) || session.selectedLayerIds.length !== 1} onClick={() => { workspace.beginLayerTransform(); closeMenu() }}>{t('app.menu.edit.transform')} <kbd>{shortcutFor('transform')}</kbd></button><button disabled={!session?.selection} onClick={() => { workspace.deleteSelection(); closeMenu() }}>{t('app.menu.edit.deleteSelection')} <kbd>{shortcutFor('deleteLayer')}</kbd></button><button disabled={!session?.selection} onClick={() => { onOpenOutline(); closeMenu() }}>{t('app.menu.edit.outlineSelection')} <kbd>{shortcutFor('outline')}</kbd></button><button disabled={!session?.selection} onClick={() => { workspace.invertSelection(); closeMenu() }}>{t('app.menu.edit.invertSelection')}{shortcutHint('invertSelection')}</button><button disabled={!session?.selection} onClick={() => { if (session?.selection) workspace.commitSelectionChange(session.selection, null, t('app.menu.edit.deselect')); closeMenu() }}>{t('app.menu.edit.deselect')} <kbd>{shortcutFor('deselect')}</kbd></button><span className="menu-divider" /><div className="menu-submenu"><button className="menu-submenu-trigger" disabled={!session}>{t('app.menu.edit.adjustments')} <ChevronRight size={14} /></button><div className="menu-popover menu-submenu-popover"><button onClick={() => openAdjustment('color-balance')}>{t('app.menu.edit.colorBalance')}{shortcutHint('adjustmentColorBalance')}</button><button onClick={() => openAdjustment('brightness-contrast')}>{t('app.menu.edit.brightnessContrast')}{shortcutHint('adjustmentBrightnessContrast')}</button><button onClick={() => openAdjustment('hue-saturation')}>{t('app.menu.edit.hueSaturation')}{shortcutHint('adjustmentHueSaturation')}</button><button onClick={() => openAdjustment('curves')}>{t('app.menu.edit.curves')}{shortcutHint('adjustmentCurves')}</button></div></div><span className="menu-divider" /><button onClick={() => { onOpenShortcuts(); closeMenu() }}>{t('app.menu.edit.shortcuts')}{shortcutHint('openShortcutSettings')}</button><button onClick={() => { onOpenPreferences(); closeMenu() }}>{t('app.menu.edit.preferences')}{shortcutHint('openPreferences')}</button></div>}</div>
      <div className="menu-item"><button aria-expanded={openMenu === 'canvas'} onClick={() => toggleMenu('canvas')}>{t('app.menu.image')}</button>{openMenu === 'canvas' && <div className="menu-popover"><button disabled={!session} onClick={() => { onOpenCanvasResize(); closeMenu() }}>{t('app.menu.image.canvasSize')} <kbd>{shortcutFor('canvasResize')}</kbd></button><button disabled={!session} onClick={() => { onOpenImageResize(); closeMenu() }}>{t('app.menu.image.imageSize')} <kbd>{shortcutFor('imageResize')}</kbd></button><button disabled={!session} onClick={() => { if (session) void workspace.convertColorMode(session.document.colorMode === 'rgba' ? 'indexed' : 'rgba'); closeMenu() }}>{t(session?.document.colorMode === 'rgba' ? 'app.menu.image.toIndexed' : 'app.menu.image.toRgba')}{shortcutHint('convertColorMode')}</button></div>}</div>
      <div className="menu-item"><button aria-expanded={openMenu === 'layer'} onClick={() => toggleMenu('layer')}>{t('app.menu.layer')}</button>{openMenu === 'layer' && <div className="menu-popover"><button disabled={!session} onClick={() => { void workspace.addLayer(); closeMenu() }}>{t('app.menu.layer.new')}{shortcutHint('newLayer')}</button><button disabled={!session} onClick={() => { workspace.createLayerGroup(); closeMenu() }}>{t('app.menu.layer.newGroup')} <kbd>{shortcutFor('createLayerGroup')}</kbd></button><button disabled={!session || Boolean(session.selectedGroupId)} onClick={() => { workspace.duplicateActiveLayer(); closeMenu() }}>{t('app.menu.layer.duplicate')}{shortcutHint('duplicateLayer')}</button><button disabled={!session || Boolean(session.selectedGroupId)} onClick={() => { workspace.mergeActiveLayerDown(); closeMenu() }}>{t('app.menu.layer.mergeDown')}{shortcutHint('mergeLayerDown')}</button><button disabled={!session || Boolean(session.selectedGroupId) || session.selectedLayerIds.length < 2} onClick={() => { workspace.mergeSelectedLayers(); closeMenu() }}>{t('app.menu.layer.mergeSelected')}{shortcutHint('mergeSelectedLayers')}</button><button disabled={!session?.selectedGroupId} onClick={() => { workspace.mergeSelectedGroup(); closeMenu() }}>{t('app.menu.layer.mergeGroup')}{shortcutHint('mergeLayerGroup')}</button><button disabled={!session || session.document.layers.length < 2} onClick={() => { workspace.mergeVisibleLayers(); closeMenu() }}>{t('app.menu.layer.mergeVisible')}{shortcutHint('mergeVisibleLayers')}</button><button disabled={!session?.selectedGroupId} onClick={() => { workspace.ungroupSelected(); closeMenu() }}>{t('app.menu.layer.ungroup')} <kbd>{shortcutFor('ungroupLayers')}</kbd></button><button disabled={!session?.selection} onClick={() => { workspace.toggleSelectionOutline(); closeMenu() }}>{t(session?.view.showSelectionOutline === false ? 'app.menu.layer.showOutline' : 'app.menu.layer.hideOutline')}{shortcutHint('toggleSelectionOutline')}<span className="menu-check">{session?.view.showSelectionOutline === false && <Check size={14} />}</span></button></div>}</div>
      <div className="menu-item"><button aria-expanded={openMenu === 'window'} onClick={() => toggleMenu('window')}>{t('app.menu.window')}</button>{openMenu === 'window' && <div className="menu-popover">
        ...
        <span className="menu-divider" />
        <div className="menu-submenu"><button className="menu-submenu-trigger" disabled={!session}>{t('app.menu.window.panels')} <ChevronRight size={14} /></button><div className="menu-popover menu-submenu-popover">{([['color', t('app.menu.window.panelColor'), 'toggleColorPanel'], ['palette', t('app.menu.window.panelPalette'), 'togglePalettePanel'], ['layers', t('app.menu.window.panelLayers'), 'toggleLayersPanel'], ['preview', t('app.menu.window.panelPreview'), 'togglePreviewPanel']] as Array<[WorkspacePanelId, string, string]>).map(([id, label, shortcutId]) => <button key={id} disabled={!session} onClick={() => { onTogglePanel(id); closeMenu() }}>{label}{shortcutHint(shortcutId)}<span className="menu-check">{panelVisibility[id] && <Check size={14} />}</span></button>)}</div></div>
        <div className="menu-submenu"><button className="menu-submenu-trigger" disabled={!session}>{t('app.menu.window.toolRailPosition')} <ChevronRight size={14} /></button><div className="menu-popover menu-submenu-popover"><button disabled={!session} onClick={() => { onToolRailSideChange('left'); closeMenu() }}>{t('app.menu.window.left')}{shortcutHint('toolRailLeft')}<span className="menu-check">{toolRailSide === 'left' && <Check size={14} />}</span></button><button disabled={!session} onClick={() => { onToolRailSideChange('right'); closeMenu() }}>{t('app.menu.window.right')}{shortcutHint('toolRailRight')}<span className="menu-check">{toolRailSide === 'right' && <Check size={14} />}</span></button></div></div>
        <span className="menu-divider" />
      ...
      <div className="menu-item"><button aria-expanded={openMenu === 'help'} onClick={() => toggleMenu('help')}>{t('app.menu.help')}</button>{openMenu === 'help' && <div className="menu-popover"><button onClick={() => { onOpenComponentLibrary(); closeMenu() }}>{t('app.menu.help.componentLibrary')}{shortcutHint('openComponentLibrary')}</button><button onClick={() => { onOpenLatestRelease(); closeMenu() }}>{t('app.menu.help.changelog')}</button><button onClick={() => { onOpenRoadmap(); closeMenu() }}>{t('app.menu.help.roadmap')}</button><button onClick={() => { onOpenAbout(); closeMenu() }}>{t('app.menu.help.about')}{shortcutHint('openAbout')}</button></div>}</div>
    </nav>
    <div className="top-actions"><button className="icon-button" title={`${t('app.menu.file.new')} ${shortcutFor('newDocument')}`.trim()} aria-label={t('app.menu.file.new')} onClick={onNew}><Plus size={17} /></button><button className="icon-button" title={`${t('app.menu.file.open')} ${shortcutFor('openDocument')}`.trim()} aria-label={t('app.menu.file.open')} onClick={onOpen}><FolderOpen size={17} /></button><button className="icon-button" title={`${t('app.menu.file.save')} ${shortcutFor('save')}`.trim()} aria-label={t('app.menu.file.save')} disabled={!session} onClick={() => void workspace.saveActive()}><Save size={17} /></button><button className="top-export-button" title={`${t('app.menu.file.export')} ${shortcutFor('exportDocument')}`.trim()} disabled={!session} onClick={onExport}><FileOutput size={16} /><span>{t('app.menu.file.export')}</span></button></div>
