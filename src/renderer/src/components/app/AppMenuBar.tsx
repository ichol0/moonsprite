...
export function AppMenuBar({
  ...
  return <header className="topbar">
    ...
    <nav className="menu-strip" aria-label="主菜单">
      <div className="menu-item"><button aria-expanded={openMenu === 'file'} onClick={() => toggleMenu('file')}>文件</button>{openMenu === 'file' && <div className="menu-popover"><button onClick={() => { onNew(); closeMenu() }}>新建 <kbd>{shortcutFor('newDocument')}</kbd></button><button onClick={() => { onOpen(); closeMenu() }}>打开 <kbd>{shortcutFor('openDocument')}</kbd></button><button disabled={!session} onClick={() => { void workspace.saveActive(); closeMenu() }}>保存 <kbd>{shortcutFor('save')}</kbd></button><button disabled={!session} onClick={() => { onSaveAs(); closeMenu() }}>另存为 <kbd>{shortcutFor('saveAs')}</kbd></button><button disabled={!session} onClick={() => { onExport(); closeMenu() }}>导出 <kbd>{shortcutFor('exportDocument')}</kbd></button><button disabled={!session?.document.filePath && !session?.document.sourceFilePath} onClick={() => { if (session) onOpenProjectFolder(session.document.id); closeMenu() }}>在文件夹中打开</button></div>}</div>
      <div className="menu-item"><button aria-expanded={openMenu === 'edit'} onClick={() => toggleMenu('edit')}>编辑</button>{openMenu === 'edit' && <div className="menu-popover"><button disabled={!session?.history.canUndo} onClick={() => { workspace.undo(); closeMenu() }}>撤销 <kbd>{shortcutFor('undo')}</kbd></button><button disabled={!session?.history.canRedo} onClick={() => { workspace.redo(); closeMenu() }}>重做 <kbd>{shortcutFor('redo')}</kbd></button><button disabled={!session || Boolean(session.selectedGroupId) || session.selectedLayerIds.length !== 1} onClick={() => { workspace.beginLayerTransform(); closeMenu() }}>变换 <kbd>{shortcutFor('transform')}</kbd></button><button disabled={!session?.selection} onClick={() => { workspace.deleteSelection(); closeMenu() }}>删除选区 <kbd>{shortcutFor('deleteLayer')}</kbd></button><button disabled={!session?.selection} onClick={() => { onOpenOutline(); closeMenu() }}>选区描边 <kbd>{shortcutFor('outline')}</kbd></button><button disabled={!session?.selection} onClick={() => { if (session?.selection) workspace.commitSelectionChange(session.selection, null, '取消选区'); closeMenu() }}>取消选择 <kbd>{shortcutFor('deselect')}</kbd></button><span className="menu-divider" /><div className="menu-submenu"><button className="menu-submenu-trigger" disabled={!session}>调整 <ChevronRight size={14} /></button><div className="menu-popover menu-submenu-popover"><button onClick={() => openAdjustment('color-balance')}>色彩平衡</button><button onClick={() => openAdjustment('brightness-contrast')}>亮度/对比度</button><button onClick={() => openAdjustment('hue-saturation')}>色相/饱和度</button><button onClick={() => openAdjustment('curves')}>曲线</button></div></div><span className="menu-divider" /><button onClick={() => { onOpenShortcuts(); closeMenu() }}>快捷键设置</button><button onClick={() => { onOpenPreferences(); closeMenu() }}>首选项</button></div>}</div>
      <div className="menu-item"><button aria-expanded={openMenu === 'canvas'} onClick={() => toggleMenu('canvas')}>图像</button>{openMenu === 'canvas' && <div className="menu-popover"><button disabled={!session} onClick={() => { onOpenCanvasResize(); closeMenu() }}>调整画布尺寸 <kbd>{shortcutFor('canvasResize')}</kbd></button><button disabled={!session} onClick={() => { onOpenImageResize(); closeMenu() }}>调整图像尺寸 <kbd>{shortcutFor('imageResize')}</kbd></button><button disabled={!session} onClick={() => { if (session) void workspace.convertColorMode(session.document.colorMode === 'rgba' ? 'indexed' : 'rgba'); closeMenu() }}>{session?.document.colorMode === 'rgba' ? '转换为索引模式' : '转换为 RGBA 模式'}</button></div>}</div>
