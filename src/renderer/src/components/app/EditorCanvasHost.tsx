  ...
  const renderNode = (node: DocumentPaneNode): ReactNode => {
    if (node.kind === 'leaf') {
      ...
      return <section key={node.id} data-document-pane-id={node.id} className={`document-pane ${activeId === paneSession.document.id ? 'active' : ''} ${draggingPaneId === paneSession.document.id ? 'dragging' : ''}`} onPointerDownCapture={() => useWorkspace.getState().setActive(paneSession.document.id)} onWheelCapture={() => useWorkspace.getState().setActive(paneSession.document.id)}>
        <header onPointerDown={(event) => beginPaneDrag(event, paneSession.document.id)}><PixelUtilityIcon kind="image" /><span>{paneSession.document.name}</span>{paneSession.document.dirty && <i />}<button title={t('common.close')} aria-label={t('tabs.closeAria', { name: paneSession.document.name })} onClick={() => { void useWorkspace.getState().closeDocument(paneSession.document.id) }}><PixelUtilityIcon kind="close" /></button></header>
