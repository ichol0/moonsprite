...
function panelDockZoneAt(clientX: number, clientY: number): PanelDockZone | null {
  const contains = (bounds: DOMRect): boolean => clientX >= bounds.left && clientX <= bounds.right && clientY >= bounds.top && clientY <= bounds.bottom
  const compactPreview = (dock: FixedPanelDock, bounds: DOMRect): React.CSSProperties => {
    if (dock === 'bottom') {
...
function useFloatingPanel(initialPosition: FloatingPosition | null = null, followViewportRight = false, canDock = true, storageKey?: string, responsiveToViewport = false, onDock?: (dock: FixedPanelDock) => void, forceDocked = false) {
  ...
  useEffect(() => {
    const move = (event: PointerEvent): void => {
      if (panelResize.current) {
        ...
        if (start.direction.includes('w')) {
          width = Math.max(minimumWidth, Math.min(start.x + start.width, start.width - deltaX))
          x = start.x + start.width - width
        }
        if (start.direction.includes('n')) {
          height = Math.max(minimumHeight, Math.min(start.y + start.height, start.height - deltaY))
      ...
      const x = Math.max(-width + 160, Math.min(window.innerWidth - 120, event.clientX - drag.current.offsetX))
      const y = Math.max(0, Math.min(window.innerHeight - 32, event.clientY - drag.current.offsetY))
      updatePosition((current) => ({ x, y, width: current?.width ?? width, height: current?.height ?? height }))
      if (canDock) {
    ...
    const resize = (): void => {
      updatePosition((current) => {
        ...
        const x = responsiveToViewport
          ? current.x * scaleX
          : followViewportRight && !userPositioned.current ? window.innerWidth - initialRightOffset.current : current.x
    ...
    return () => { window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', up); window.removeEventListener('resize', resize) }
  }, [])
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      ...
      updatePosition((current) => {
        ...
        const widthUnchanged = width === undefined || Math.abs(width - current.width!) < 1
        const heightUnchanged = height === undefined || Math.abs(height - current.height!) < 1
        if (widthUnchanged && heightUnchanged) return current
  ...
  const startDrag = (event: React.PointerEvent<HTMLElement>, allowMiddle = false): void => {
    ...
    updatePosition((current) => current ?? { x: panelBounds.left, y: panelBounds.top, width: panelBounds.width, height: panelBounds.height })
    drag.current = { offsetX: event.clientX - panelBounds.left, offsetY: event.clientY - panelBounds.top }
    event.preventDefault()
  }

  const startResize = (event: React.PointerEvent<HTMLElement>, direction: ResizeDirection): void => {
  ...
  const startDetachedDrag = (clientX: number, clientY: number, continueDrag = true): void => {
    ...
    const offsetY = 16
    const next = {
      x: Math.max(-bounds.width + 160, Math.min(window.innerWidth - 120, clientX - offsetX)),
      y: Math.max(0, Math.min(window.innerHeight - 32, clientY - offsetY)),
      width: bounds.width,
      height: bounds.height
    }
    updatePosition(() => next)
    persistPosition(next)
    drag.current = continueDrag ? { offsetX, offsetY } : null
  }
  ...
  const resizeTo = (width: number, height: number): void => {
    ...
    updatePosition((current) => {
      if (!current) return current
      const nextWidth = Math.max(180, Math.min(window.innerWidth - current.x, width))
      const nextHeight = Math.max(120, Math.min(window.innerHeight - current.y, height))
      nextPosition = { ...current, width: nextWidth, height: nextHeight }
      return nextPosition
    })
    if (nextPosition) persistPosition(nextPosition)
  }
  const style: React.CSSProperties | undefined = position ? { position: 'fixed', left: position.x, top: position.y, width: position.width, height: position.height, zIndex } : undefined
  return { ref, style, dockPreview, startDrag, startDetachedDrag, startResize, resizeTo, bringToFront: () => setZIndex(++floatingZIndex) }
}

function PanelResizeHandles({ onResize }: { onResize: (event: React.PointerEvent<HTMLElement>, direction: ResizeDirection) => void }) {
...
function FloatingDockPreview({ style }: { style: React.CSSProperties | null }) {
...
type PaletteSwatchSize = 'small' | 'medium' | 'large'
const paletteSwatchPixels: Record<PaletteSwatchSize, number> = { small: 22, medium: 30, large: 40 }
const paletteColorsEqual = (left: RgbaColor[], right: RgbaColor[]): boolean =>
  left.length === right.length && left.every((color, index) => colorEquals(color, right[index]))
const paletteMarkerColor = (color: RgbaColor): string => {
...
const reorderPalettePreview = (order: number[], ids: number[], targetSlot: number): number[] => {
...
const paletteReorderTarget = (order: number[], ids: number[], preview: number[]): { id: number; insertAfter: boolean } | null => {
  const selected = new Set(ids)
