...
type PreferenceSection = 'general' | 'files' | 'cursor' | 'background' | 'editing' | 'presets' | 'reset'
...
export function PreferencesDialog({ onClose, onPresetChange }: PreferencesDialogProps) {
  ...
  const persist = (): void => {
    ...
    saveEditorPreferences({ ...preferences, documentSizePresets, exportScalePresets })
    window.dispatchEvent(new Event('moonsprite:preferences-changed'))
    onPresetChange(documentSizePresets, exportScalePresets)
  }
  const toggle = (label: string, checked: boolean, onChange: (checked: boolean) => void) => <label className="preference-toggle outline-preview-toggle"><span>{label}</span><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /><span className="toggle-track" aria-hidden="true"><i /></span></label>
  ...
  return <div className="modal-backdrop" role="presentation"><section className="modal settings-modal" role="dialog" aria-label="首选项">
    ...
    <div className="settings-layout"><nav>{([['general', '常规'], ['files', '文件'], ['cursor', '光标'], ['background', '背景'], ['editing', '编辑'], ['presets', '预设'], ['reset', '重置']] as Array<[PreferenceSection, string]>).map(([id, label]) => <button key={id} className={section === id ? 'selected' : ''} onClick={() => setSection(id)}>{label}</button>)}</nav><main>
      {section === 'general' && <>
        <label className="preference-field">语言<ThemedSelect value={preferences.language} groups={[{ label: '语言', options: [{ value: 'zh-CN', label: '简体中文' }, { value: 'en-US', label: 'English' }] }]} label="语言" onChange={(value) => update('language', value)} /></label>
      ...
      {section === 'cursor' && <>
        {toggle('优先使用本地鼠标指针（缺失时使用软件指针）', preferences.useLocalCursors, (value) => update('useLocalCursors', value))}
