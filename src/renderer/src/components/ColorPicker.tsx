  ...
  return <div ref={pickerRef} className={`moon-color-picker ${compact ? 'compact' : ''}`}>
    <div className="integrated-color-picker">
      <div className={`color-spectrum-stack scheme-${scheme} ${moonTriangle ? 'moon-triangle' : ''} ${scheme === 'wheel' && colorSteps > 0 ? 'quantized-wheel' : ''}`}>
       ...
       <ColorValueControl color={color} onChange={onChange} label={effectiveLabel} roleLabel={t('colorPicker.foreground')} storageKey="palette-foreground" fillWithColor inPalette={paletteColors?.some((entry) => entry.r === color.r && entry.g === color.g && entry.b === color.b && entry.a === color.a)} onAddToPalette={onAddPaletteColor ? () => onAddPaletteColor(color) : undefined} />
       {secondaryColor && onSecondaryChange && <ColorValueControl color={secondaryColor} onChange={onSecondaryChange} label={effectiveLabel} roleLabel={t('colorPicker.background')} storageKey="palette-background" className="color-secondary-value-control" fillWithColor inPalette={paletteColors?.some((entry) => entry.r === secondaryColor.r && entry.g === secondaryColor.g && entry.b === secondaryColor.b && entry.a === secondaryColor.a)} onAddToPalette={onAddPaletteColor ? () => onAddPaletteColor(secondaryColor) : undefined} />}
     </div>
  </div>
}
