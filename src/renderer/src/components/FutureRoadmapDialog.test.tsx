...
describe('FutureRoadmapDialog', () => {
  it('shows every planned feature and marks implemented items as completed', () => {
    ...
    expect(items.slice(0, 16).every((item) => item.classList.contains('completed'))).toBe(true)
    expect(items.slice(16).every((item) => !item.classList.contains('completed'))).toBe(true)
  })
