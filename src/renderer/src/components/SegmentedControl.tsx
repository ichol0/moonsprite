...
export function SegmentedControl<T extends string>({ className = '', label, onChange, options, value }: SegmentedControlProps<T>) {
  return <div className={`segmented-control ${className}`.trim()} role="group" aria-label={label}>
    {options.map((option) => {
      const button = <button key={option.value} type="button" className={option.value === value ? 'selected' : ''} aria-pressed={option.value === value} disabled={option.disabled} onClick={() => onChange(option.value)}>{option.label}</button>
