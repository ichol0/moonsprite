...
import { anchoredPreviewPan, pixelPerfectPreviewScale, previewCheckerCellSize } from '@/core/preview-geometry'
...
export function PreviewPanel({ session, onClose, docked = false, onDockDragStart, onPanelContextMenu, onFloatingDock, relativeLuminanceInPreview = true }: { session: DocumentSession; onClose: () => void; relativeLuminanceInPreview?: boolean } & DockDragProps) {
  ...
  const sourceRef = useRef<{ documentId: string; revision: number; frameId: string; relativeLuminance: boolean; canvas: OffscreenCanvas } | null>(null)
  ...
  const zoomLabel = `${Number((zoom * 100).toFixed(2))}%`
  ...
  useEffect(() => {
    ...
    if (!source || source.documentId !== session.document.id || source.revision !== session.revision || source.frameId !== previewFrameId || source.relativeLuminance !== showRelativeLuminance) {
      ...
      const sampleComposite = createCompositeSampler(previewDocument)
      for (let index = 0; index < width * height; index += 1) {
        const offset = index * 4
        const sampled = sampleComposite(index)
      ...
      source = { documentId: session.document.id, revision: session.revision, frameId: previewFrameId, relativeLuminance: showRelativeLuminance, canvas: sourceCanvas }
    ...
    const draw = (): void => {
      ...
      const scale = pixelPerfectPreviewScale(baseFit.scale * zoom)
      ...
      context.imageSmoothingEnabled = scale < 0.5
  ...
  }, [session.document, session.revision, previewFrameId, showRelativeLuminance, checkerboard, zoom, pan])
  ...
  return <section ref={floating.ref} className={`panel preview-panel ${floating.style ? 'floating-panel' : ''}`} style={floating.style} onPointerDown={floating.bringToFront} onContextMenu={onPanelContextMenu}>
    <header onPointerDown={(event) => floating.style ? floating.startDrag(event) : onDockDragStart?.(event, floating.startDetachedDrag)}><span>{t('panel.preview')}</span><small>{zoomLabel}</small><span className="panel-actions"><button title={t('preview.zoomOut')} aria-label={t('preview.zoomOut')} onClick={() => adjustZoom(false)}><Minus size={14} /></button><button title={t('preview.zoomIn')} aria-label={t('preview.zoomIn')} onClick={() => adjustZoom(true)}><Plus size={14} /></button><button className={previewPlaying ? 'active' : ''} disabled={timeline.frames.length <= 1} title={t(previewPlaying ? 'timeline.pause' : 'timeline.play')} aria-label={t(previewPlaying ? 'timeline.pause' : 'timeline.play')} onClick={() => setPreviewPlayingState(!previewPlaying)} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); setPlaybackMenu({ x: event.clientX, y: event.clientY }) }}>{previewPlaying ? <Pause size={14} /> : <Play size={14} />}</button><button title={t('preview.close')} aria-label={t('preview.close')} onClick={onClose}><X size={14} /></button></span></header>
    <div className="preview-canvas-wrap" onWheel={(event) => { const bounds = canvasRef.current?.getBoundingClientRect(); if (!bounds) return; event.preventDefault(); adjustZoom(event.deltaY < 0, { x: event.clientX - bounds.left, y: event.clientY - bounds.top }) }} onPointerDown={(event) => { if (event.button !== 1) return; panDrag.current = { x: event.clientX, y: event.clientY, panX: pan.x, panY: pan.y }; event.currentTarget.setPointerCapture(event.pointerId); event.preventDefault() }} onPointerMove={(event) => { const drag = panDrag.current; if (!drag) return; schedulePan({ x: drag.panX + event.clientX - drag.x, y: drag.panY + event.clientY - drag.y }) }} onPointerUp={(event) => { panDrag.current = null; if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId) }} onPointerCancel={() => { panDrag.current = null }}><canvas ref={canvasRef} aria-label={t('preview.canvasAria')} /></div>
