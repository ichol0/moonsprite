...
export function ThemedSelect<T extends string>({ value, groups, label, onChange }: {
  value: T
  ...
  return <span className="themed-select">
    <button ref={triggerRef} type="button" className="themed-select-trigger" aria-label={label} aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((current) => !current)} onKeyDown={(event) => {
